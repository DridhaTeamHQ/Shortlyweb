...
// Live, animated desi-maximalism preview of the edition the reader is building.
// Reacts to rhythm / days / categories / source / name as they select on the left.
...
export default function EditionPreview({ rhythm = 'daily', days = [], source = 'top', categories = [], name = '', summary = '' }) {
  // The most recently picked category tints the whole edition.
  const lead = categories.length ? themeBySlug[categories[categories.length - 1]] : null
  const grad = lead
    ? `linear-gradient(135deg, ${lead.desi.from}, ${lead.desi.to})`
    : 'linear-gradient(135deg, #F4A300, #D81B60)'
  const accent = lead?.desi.accent || '#F4A300'
  ...
  return (
    <div className="desi-frame relative overflow-hidden rounded-3xl bg-[#fffdf5]">
      ...
      <div className="relative overflow-hidden px-6 pb-9 pt-7 text-white">
        <AnimatePresence>
          <motion.div
        ...
        <div className="relative">
          <div className="text-[10px] font-bold uppercase tracking-[0.3em]" style={{ ...SANS, color: accent }}>
            ✦ Shortly Edition ✦
          </div>
          ...
          <p className="mt-1 text-sm text-white/90" style={SANS}>
            Namaste, {name?.trim() || 'Reader'}
          </p>
          <span
            className="mt-3 inline-block rounded-full px-3 py-1 text-[11px] font-bold text-[#3a1206]"
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-2">
            {[0, 1, 2].map((i) => (
              <motion.div
                key={i}
                animate={{ opacity: i < breadth ? 1 : 0.18 }}
      ...
      <div className="desi-jhalar" style={{ '--jhalar': accent }} />
    </div>
  )
}
