...
void main() {
  ...
  // Prevent negative intensity which can introduce dark/black regions
  float intensity = max(0.0, 0.6 * height);
  
  float midPoint = uMidPoint;
  float auroraAlpha = smoothstep(midPoint - uBlend * 0.5, midPoint + uBlend * 0.5, intensity);
  // Couple alpha with intensity so low-intensity pixels remain transparent
  auroraAlpha *= smoothstep(0.0, 1.0, intensity);
...
export default function Aurora(props) {
  const { colorStops = ['#5227FF', '#7cff67', '#5227FF'], amplitude = 1.0, blend = 0.5, speed = 1.0, midPoint = 0.20 } = props;
  ...
  useEffect(() => {
    ...
    const update = t => {
      ...
      program.uniforms.uMidPoint.value = propsRef.current.midPoint ?? midPoint;
  ...
  }, [amplitude, blend, speed, midPoint, colorStops]);
